...
Chart.prototype.load = function (chartInfo, done) {
  ...
  jsChartFrame['p:graphicFrame']['p:nvGraphicFramePr']['p:nvPr']['p:extLst'][
    'p:ext'
  ]['p14:modId'] = {
    $: {
  ...
  this.createWorkbook(chartInfo, function (err, workbookJSZip) {
    self.presentation.registerChartWorkbook(
      chartName,
      workbookJSZip.generate({ type: 'arraybuffer' }),
    );
    done(null, self);
  });
};
...
Chart.prototype.getChartBase = function (chartInfo) {
  ...
  jsChart['c:chartSpace']['c:chart'][0]['c:plotArea'][0]['c:barChart'][0][
    'c:ser'
  ] = jsChartSeries['c:ser'];
  return jsChart;
};
...
Chart.prototype._ser = function (serie, i) {
  ...
  var ser = XmlNode()
    ...
    .addChild(
      'c:tx',
      this._strRef('Sheet1!' + rc2a(1, 2 + i, true, true), [serie.name]),
    )
  ...
  if (serie.color) {
    ser.addChild(
      ...
      XmlNode().addChild(
        'a:solidFill',
        XmlNode().addChild('a:srgbClr', XmlNode().attr('val', serie.color)),
      ),
    );
...
Chart.prototype._strCache = function (stringArr) {
  var strRef = XmlNode().addChild(
    'c:ptCount',
    XmlNode().attr('val', stringArr.length),
  );
  for (var i = 0; i < stringArr.length; i++) {
    strRef.addChild(
      'c:pt',
      XmlNode().attr('idx', i).addChild('c:v', stringArr[i]),
    );
  }
...
Chart.prototype._numRef = function (region, numArr, formatCode) {
  ...
  for (var i = 0; i < numArr.length; i++) {
    numCache.addChild(
      'c:pt',
      XmlNode().attr('idx', i).addChild('c:v', numArr[i].toString()),
    );
  }

  var numRef = XmlNode().addChild(
    'c:numRef',
...
Chart.prototype._rowColToSheetAddress = function (
  row,
  col,
  isRowAbsolute,
  isColAbsolute,
