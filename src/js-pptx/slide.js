...
Slide.prototype.getShapes = function () {
  ...
  return this.content['p:sld']['p:cSld'][0]['p:spTree'][0]['p:sp']?.map(
    function (sp) {
      return new Shape(sp);
    },
  );
...
Slide.prototype.addChart = function (data, done) {
  ...
  chart.load(data, function (err, data) {
    // TODO pass it real data
    self.content['p:sld']['p:cSld'][0]['p:spTree'][0]['p:graphicFrame'] =
      chart.content; //jsChartFrame["p:graphicFrame"];

    // Add entry to slide1.xml.rels
    // There should a slide-level and/or presentation-level method to add/track rels
    var rels =
      self.presentation.content['ppt/slides/_rels/' + slideName + '.xml.rels'];
